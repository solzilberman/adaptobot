...
def evaluate_battery_utility_func(utilityFunc, dp):
    return utilityFunc.func(dp[0], dp[1])

def evaluate_cv_utility_func(utilityFunc, dp):
    return utilityFunc.func(dp, False)


utility_map = {
    "M0-S1.1.1" : lambda uf,dp: evaluate_battery_utility_func(uf, dp),
    "M0-S1.2.1" : lambda uf,dp: evaluate_cv_utility_func(uf, dp),
}
...
class SAS:
    ...
    def _monitor(self):
        ...
        # map each util func to request to rover
    ...
    def _plan(self):
        ...
        if self.rover.shouldUpdateDirection:
            self.rover.planFuncs.append("self.update_direction_to_waypoint()")
            self.rover.planFuncs.append("self.shouldUpdateDirection = False")
